...
import { getOrderNumberString, getGeneratedFilenameByDate } from "helpers/objectHelpers";
...
const NewPostForm = (props) => {
    ...
    const handleUpdateThumbnailFilename = () => {
        const date = new Date(timestampInputRef.current.value);
        const orderNumberString = getOrderNumberString(orderNumberInputRef.current.value);
        thumbnailFilenameInputRef.current.value = getGeneratedFilenameByDate(date, orderNumberString);
    };

    const handleUpdateNorwegianSlug = () => {
        slugNoInputRef.current.value = convertToUrlFriendlyString(titleNoInputRef.current.value);
    };

    const handleUpdateEnglishSlug = () => {
        slugEnInputRef.current.value = convertToUrlFriendlyString(titleEnInputRef.current.value);
    };
    ...
    const handleSubmit = (event) => {
        ...
        const postData = {
            ...
            link:
                !!linkInternalInputRef?.current?.checked || !!linkInternalInputRef?.current?.value
                    ? {
            ...
            <div className={formStyle.formElement}>
                <label htmlFor="post-timestamp">
                    ...
                    <input
                        ...
                        required
                        onChange={handleUpdateThumbnailFilename}
